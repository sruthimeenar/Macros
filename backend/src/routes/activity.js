...
router.get("/today", requireAuth, (req, res) => {
  res.json({ activity: db.getActivity(req.user.id, today()) });
});
...
router.post("/manual", requireAuth, (req, res) => {
  ...
  const existing = db.getActivity(req.user.id, today()) || {};
  const merged = {
  ...
  db.saveActivity(req.user.id, today(), merged);
  res.json({ activity: db.getActivity(req.user.id, today()) });
});
...
router.post("/sync", requireAuth, async (req, res) => {
  const connected = db.listDeviceProviders(req.user.id);
  ...
  try {
    ...
    db.saveActivity(req.user.id, today(), best.data);
    res.json({ activity: db.getActivity(req.user.id, today()) });
  } catch (err) {
    console.error(err);
