...
router.post("/signup", (req, res) => {
  ...
  if (db.getUserByEmail(normalizedEmail)) return res.status(409).json({ error: "An account with this email already exists." });

  const salt = genSalt();
  const hash = hashPassword(password, salt);
  const user = db.createUser({ email: normalizedEmail, name: String(name).trim(), passwordHash: hash, passwordSalt: salt });
