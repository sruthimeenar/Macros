...
router.get("/:provider/callback", async (req, res) => {
  ...
  try {
    ...
    await db.saveDeviceTokens(payload.userId, providerName, tokens);
...
router.get("/status", requireAuth, async (req, res) => {
  res.json({ connected: await db.listDeviceProviders(req.user.id) });
});

router.delete("/:provider", requireAuth, async (req, res) => {
  await db.deleteDeviceTokens(req.user.id, req.params.provider);
  res.json({ ok: true });
});
