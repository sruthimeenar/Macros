...
## Storage
Tokens are stored in `data/tokens.json`, a plain JSON file which is fine for a personal project, but swap in a real database (Postgres, SQLite, etc.) before this handles more than a handful of users, since concurrent writes to a single JSON file aren't safe at scale.
