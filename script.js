...
function renderPlan() {
  const profile = JSON.parse(get("profile", "{}"));
  if (!profile.goal || get("planReady") !== "true") { window.location.replace("index.html"); return; }
...
window.addToGoogleCalendar = () => { const profile = JSON.parse(get("profile", "{}")); const workouts = JSON.parse(get("weeklyWorkouts", "[]")); const today = new Date().getDay() === 0 ? 6 : new Date().getDay() - 1; const exerciseDetails = workouts[today]?.exercises || "Complete the session and mark it done in your plan."; const start = new Date(); const [hours, minutes] = (profile.wakeTime || "18:00").split(":").map(Number); start.setHours(hours, minutes, 0, 0); const end = new Date(start); end.setHours(end.getHours() + 1); const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(get("workout"))}&details=${encodeURIComponent(`Project Macros workout\n\n${exerciseDetails}`)}&dates=${formatDate(start)}/${formatDate(end)}`; window.open(url, "_blank", "noopener"); };
window.toggleTheme = () => { document.body.classList.toggle("dark-mode"); const isDark = document.body.classList.contains("dark-mode"); localStorage.setItem("theme", isDark ? "dark" : "light"); document.querySelectorAll(".theme-toggle").forEach(button => { button.innerText = isDark ? "Light" : "Dark"; }); };
function guardPlanLinks() { document.querySelectorAll("[data-plan-link]").forEach(link => link.addEventListener("click", event => { if (get("planReady") !== "true") { event.preventDefault(); alert("Please fill in Make a plan before viewing My plan."); } })); }
if (get("theme") === "dark") document.body.classList.add("dark-mode"); document.querySelectorAll(".theme-toggle").forEach(button => { button.innerText = document.body.classList.contains("dark-mode") ? "Light" : "Dark"; }); guardPlanLinks(); setupProfile(); setupProgressForm(); if (window.location.pathname.includes("plan.html")) renderPlan();
